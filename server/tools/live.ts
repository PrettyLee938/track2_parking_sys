...
for (const z of s.environment?.zones ?? []) console.log(`fans ${z.zone}: CO ${z.co === null ? "?" : z.co.toFixed(0)} ${z.risk ?? ""} -> ${z.fans_on}/${z.fans} on${z.forced ? " (CO penalty)" : ""}  [list-zones polls: ${s.environment.polls}]`);
