...
three values — `valid`, `unsigned`, `invalid` — and what to do with `unsigned` is a
setting (`GPA_REQUIRE_SIGNATURE`, default `false`). Turn it on once a level actually
signs.
...
| `GPA_REQUIRE_SIGNATURE` | `false` | drop unsigned webhooks |
