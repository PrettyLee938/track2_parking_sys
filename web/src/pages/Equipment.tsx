...
export function Equipment({ s }: { s: StateSnapshot }) {
  ...
  const zones = s.environment?.zones ?? [];
            ...
            <table className="data compact">
              ...
              <tbody>{zones.map((z) => (
                <tr key={z.zone}>
                  ...
                  <td>{z.ventilation}</td><td className="right">{z.moving}</td>
                  <td>{z.restricted ? <Badge tone="critical">Admission restricted</Badge> : <Badge tone="good">Operating</Badge>}</td>
                </tr>
              ))}</tbody>
            </table>
          </div>
