...
describe('backend HTTP boundary', () => {
  it('serves health, authentication, webhook ingestion, and arrival allocation', async () => {
    ...
    const cookies = (Array.isArray(setCookies) ? setCookies : setCookies ? [setCookies] : []).map((value) => value.split(';')[0] || '');
    const csrf = cookies.find((value) => value.startsWith('csrf='))?.slice(5) || '';
    const cookie = cookies.join('; ');
    const sessionHeaders = { cookie, 'x-csrf-token': csrf };
    expect((await app.inject({ method: 'POST', url: '/api/v1/auth/change-password', headers: sessionHeaders, payload: { password: 'new-secret' } })).statusCode).toBe(200);
