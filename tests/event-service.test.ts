...
describe('webhook event boundary', () => {
  it('accepts signed events once and reports sequence gaps', () => {
    ...
    expect(db.get<{ value: string }>('SELECT value FROM meta WHERE key = :key', { ':key': 'run_status' })?.value).toBe('reconciling');
    ...
    expect(db.get<{ value: string }>('SELECT value FROM meta WHERE key = :key', { ':key': 'run_status' })?.value).toBe('active');
  });
