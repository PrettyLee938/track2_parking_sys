...
export function registerRecoveryRoutes(app: FastifyInstance, auth: AuthService, recovery: RecoveryService) {
  ...
  app.post('/api/v1/audit/:id/correct', { schema: schema.idParams }, async (request, reply) => { const user = await adminUser(request, reply, auth); if (!user) return; recovery.correctAudit(user.id, Number((request.params as { id: string }).id), request.body as Record<string, unknown>); return { status: 'recorded' }; });
}
