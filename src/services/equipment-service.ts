...
export class EquipmentService {
  ...
  applyEvent(event: EventInput) {
    ...
    if (event.type === 'carbon_monoxide_event') this.save(`zone:${id}`, 'zone', 'observed', Number(pick(event.payload, 'CoLevel', 'coLevel', 'CarbonMonoxide') || 0), now, id);
  }
  ...
  async requestRepair(id: string, actorId?: string) {
    ...
    this.save(id, component.kind, 'under-maintenance', undefined, new Date(this.clock()).toISOString(), '');
    this.audit.record('repair-requested', 'component', id, { commandId: command.id }, actorId);
    return { id, status: 'under-maintenance' as const, commandId: command.id };
  }
  ...
  private async issueEquipmentCommand(kind: string, id: string, enabled: boolean, actorId?: string) {
    ...
    if (component && ['broken', 'under-maintenance'].includes(component.status)) throw new Error('component-unavailable');
