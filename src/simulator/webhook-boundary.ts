...
export class WebhookBoundary {
  accept(payload: Record<string, unknown>): WebhookEnvelope {
    ...
    return { valid: true, calculatedDigest, eventId, type: String(value(payload, 'Type', 'type') || 'unknown'), sequenceId: Number(value(payload, 'SequenceId', 'sequenceId') || 0), runId: runId ? String(runId) : undefined, payload, reason: undefined };
  }
}
